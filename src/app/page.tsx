"use client";

import { useState } from "react";

export default function Home() {
  const [input, setInput] = useState("");

  const appendValue = (value: string) => {
    setInput((prev) => prev + value);
  };

  const calculate = () => {
    try {
      // eslint-disable-next-line no-eval
      const result = eval(input);
      setInput(String(result));
    } catch {
      setInput("Error");
    }
  };

  const clear = () => {
    setInput("");
  };

  return (
    <main className="flex flex-col items-center justify-center min-h-screen p-6 bg-gray-100">
      <h1 className="text-3xl font-bold mb-8">Calculadora Versión 5.1</h1>

      {/* Caja gris completa */}
      <div className="bg-gray-500 p-6 rounded-lg shadow-lg w-full max-w-md">
        {/* Display superior */}
        <input
          type="text"
          className="mb-6 p-4 w-full rounded border border-gray-400 text-2xl bg-white text-black text-right"
          value={input}
          readOnly
        />

        {/* Grilla completa de 4 columnas (3 para números, 1 para operadores) */}
        <div className="grid grid-cols-4 gap-4 w-full">
          {/* Números: ocuparán las primeras 3 columnas */}
          {["1", "2", "3", "4", "5", "6", "7", "8", "9", "0", ".", ""].map((v, i) =>
            v ? (
              <button
                key={v}
                onClick={() => appendValue(v)}
                className="bg-white text-black p-4 rounded shadow text-xl hover:bg-gray-200 col-span-1"
              >
                {v}
              </button>
            ) : (
              <div key={i} className="col-span-1" /> // Espacio vacío
            )
          )}

          {/* Operadores: ubicados en la columna 4 */}
          {["+", "-", "*", "/", "C", "="].map((op, index) => (
            <button
              key={op}
              onClick={() => {
                if (op === "C") clear();
                else if (op === "=") calculate();
                else appendValue(op);
              }}
              className={`row-span-1 ${
                op === "="
                  ? "bg-green-600 hover:bg-green-700"
                  : op === "C"
                  ? "bg-red-500 hover:bg-red-600"
                  : "bg-blue-500 hover:bg-blue-600"
              } text-white p-4 rounded shadow text-xl col-start-4`}
            >
              {op}
            </button>
          ))}
        </div>
      </div>
    </main>
  );
}
